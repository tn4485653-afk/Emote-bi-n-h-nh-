...
def use_bundle():
    ...
    if not team_code or not bundle_name:
        return jsonify({"status": "error", "message": "Missing tc or name"})
    ...
    bundle_map = {
        ...
        "itachi": 914050001, "dreamspace": 914051001
    }

    name = bundle_name.lower()

    if name not in bundle_map:
        return jsonify({"status": "error", "message": f"Bundle not found: {name}"})

    b_id = bundle_map[name]

    async def run_bundle():
        global CURRENT_TEAM, IN_TEAM
        ...
        try:
            ...
            anim_pk = await animation_packet(b_id, key, iv, region)
            await SEndPacKeT(None, online_writer, 'OnLine', anim_pk)

            await asyncio.sleep(3)

            wear_pk = await bundle_packet_final(b_id, key, iv, region)
            await SEndPacKeT(None, online_writer, 'OnLine', wear_pk)

        except Exception as e:
            print("Bundle error:", e)

    asyncio.run_coroutine_threadsafe(run_bundle(), loop)
...
async def MaiiiinE():
    ...
    # BOT LOGIN UID
    BOT_UID = int('14859964045') 

    Uid, Pw = '4562804687', 'rerhh_VEZLT_BY_SPIDEERIO_GAMING_BT5P1'
